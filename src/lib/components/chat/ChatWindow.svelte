<script lang="ts">
	...
	let activeMimeTypes = $derived(
		Array.from(
			new Set([
				...mimeTypesFromActiveTools, // fetch mime types from active tools from tool settings
				...(currentModel.tools ? ["application/pdf"] : []), // if its a tool model, we can always enable document parser so we always accept pdfs
